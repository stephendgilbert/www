...
function NewSubmit(f)
{
    ...
    studentName = f.student_name.value;
    ...
    studentPercent = f.score_percent.value;
...
//    alert('correctAnswers=' + correctAnswers);
    ...
    var data = 'student_id=' + encodeURIComponent(user_id) +
        '&test_name=' + encodeURIComponent(quiz_no) +
...
//        var question = $(this).parent().parent().find('div:first');
        ...
        if (! correct) {
            question.css('background-color', '#efe');
        }
        ...
        $(this).parent().next().find('div .choice').each(function(j) {
            ...
            if (answer == correctAnswers[i]) {
                $(this).parent().next().css('border-bottom','2px dashed #3F3');
            }
            else if (! correct && answer == studentAnswers[i]) {
                $(this).parent().next().css({'background-color' : '#ff9', 'color' : 'red', 'text-decoration': 'line-through', 'border' : '1px solid red'});
            }
        });
    });

    var sr = $('tr td div.default:first');
    var html = "<div class='quiz-results'>\n" +
        "<h3>Your score: " + student_name + ", " + studentPercent + "%</h3>" +
        "<p>Print or save this page as a PDF for later study. " +
        "When you are finished, close this page.</p></div>";
    sr.html(html);
